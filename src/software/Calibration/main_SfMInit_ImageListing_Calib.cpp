...
int main(int argc, char **argv)
{
  ...
  for ( std::vector<std::string>::const_iterator iter_image = vec_image.begin();
    iter_image < vec_image.end();///Tao. changed from iter_image != vec_image.end()
    ++iter_image, ++my_progress_bar )
  {
	...
	else if(sImFilenamePart.compare(3, 1, "1") == 0)//view1
		intrinsic = std::make_shared<Pinhole_Intrinsic_Radial_K3>
            (width, height, 1268, 991.614857946361, 733.720185283513, -0.221919462753480, 0.131539090594321,-0.0651825855044944); 
	else if(sImFilenamePart.compare(3, 1, "2") == 0)//view2
		intrinsic = std::make_shared<Pinhole_Intrinsic_Radial_K3>
            (width, height, 1255, 1001.88580233165, 792.162069752880, -0.223032179371655, 0.156355226967867,-0.0946149608573477); 
	else if(sImFilenamePart.compare(3, 1, "3") == 0)//view3
		intrinsic = std::make_shared<Pinhole_Intrinsic_Radial_K3>
            (width, height, 1261.4, 1000.60061872686, 784.827148051238, -0.248628903228924, 0.293042457640324, -0.408920531360273);
