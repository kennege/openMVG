...
int main(int argc, char **argv)
{
  ...
  // std::ofstream landmarksFile;
  // landmarksFile.open("landmarks.txt");
  std::ofstream extrinsicFile;
  extrinsicFile.open("extrinsicCalibration.txt"); 
  std::ofstream intrinsicFile;
  intrinsicFile.open("intrinsicCalibration.txt");  
  int imagePcam;
  int nCameras;
  ...
  cmd.add( make_option('n', nCameras, "nCameras") );
  cmd.add( make_option('p', imagePcam, "imagesPcam") );
  ...
  try {
    ...
    << "[-f|--refineIntrinsics] Intrinsic parameters refinement option\n"
      ...
      << "[-n|--nCameras] Number of cameras to be calibrated:\n"
      << "[-p|--imagesPcam] Number of images per camera:\n"
  ...
  if (sfmEngine.Process())
  {
    ...
    SfM_Data tiny_scene = sfmEngine.Get_SfM_Data();
    Pose3 Cam0Pose(tiny_scene.GetPoses().at(0).rotation(),tiny_scene.GetPoses().at(0).center());
    Pose3 invPose = Cam0Pose.inverse();

    for (int i=0;i<imagePcam*nCameras;i+=imagePcam){
      Pose3 absPose(tiny_scene.GetPoses().at(i).rotation(),tiny_scene.GetPoses().at(i).center());
      Pose3 relPose = tiny_scene.poses[tiny_scene.views[i]->id_pose];
      vector<double> K = tiny_scene.GetIntrinsics().at(i)->getParams();
      extrinsicFile << "R:\n" << absPose.rotation()(0,0) << " " << absPose.rotation()(0,1) << " " << absPose.rotation()(0,2) << endl;
      extrinsicFile << relPose.rotation()(1,0) << " " << absPose.rotation()(1,1) << " " << absPose.rotation()(1,2) << endl;
      extrinsicFile << relPose.rotation()(2,0) << " " << absPose.rotation()(2,1) << " " << absPose.rotation()(2,2) << endl;
      extrinsicFile << "T:\n" << relPose.center()(0) << " " << absPose.center()(1) << " " << absPose.center()(2) << endl;
      intrinsicFile << "K:\n" << K[0] << " " << 0 << " " << K[1] << endl;
      intrinsicFile << 0 << " " << K[0] << " " << K[2] << endl;
      intrinsicFile << 0 << " " << 0 << " " << 1 << endl; 
      cout << i << endl; 
    }
